...
def file_too_large(e):
    return fail("CSV file is too large. Maximum size is 30 MB.", 413)
...
def upload():
    ...
    if df.shape[1] == 0:
        return fail("CSV file has no columns", 400)

    df.columns = [str(col).strip() for col in df.columns]

    if any(not col for col in df.columns):
        return fail("CSV contains blank column names", 400)

    if df.columns.duplicated().any():
        return fail("CSV contains duplicate column names", 400)
...
def train():
    ...
    for col in x_cols:
        if model_df[col].dtype == "object" or str(model_df[col].dtype) in {"string", "category", "bool"}:
